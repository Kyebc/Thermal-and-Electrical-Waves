...
class Thermal():
    ...
    def calculate_phase_lag(self):
        ...
        val = phi1 - phi2
        x = self.get_xarray()[0:int(1.5*10*self.get_period())]
        y = self.get_yarray()[0:int(1.5*10*self.get_period())]
        maxindex = np.argmax(y)
        xmaxphase = x[maxindex]
        maxphase = ((xmaxphase/self.get_period())%1)*2*np.pi
        value = (maxphase - np.pi/2)
        if value <= 0 :
            err2 = phi1 - (value + 2*np.pi)
            pass
        elif value > 0:
            err2 = phi1 - value
            pass
    ...
    def calculate_fourier_diffusivity(self, n = 1, option = "tf"):
        ...
        if option == "tf":
            ...
            if amplitude_outer != 0:
                ...
                # print(transmission_factor) # uncomment to print 
            ...
            if n%2 != 0:
                ...
                # print(phase_lag) # uncomment to print
    ...
    #very inefficient loop, kept as reference
    def discarded_function_plot_fourier(self, n = 1):
        x = []
        y = []
    ...
    def plot_fourier(self, n = 1):
        ...
        plt.figure()
        plt.title("Fourier fit:" + self.get_file_name())
        plt.xlabel("Time/seconds")
...
def return_fourier_fit(array, n = 3):
    for i in range(1, len(array)):
        eval(str(array[i]) + ".plot_fourier(" + str(n) + ")")
        pass
    plt.figure()
    for i in range(0,len(array)): # run through different datasets
        Dtfs = []
        Dtfe = []
        Dpls = []
        Dple = []
        periodtf = []
        periodpl = []
        for j in range(1, n + 1): # run through different values of n's
            Dtf = eval(str(array[i]) + ".calculate_fourier_diffusivity(n = " + str(j) + ", option = \"tf\")")
            if j%2 == 1:
                Dtfs.append(Dtf[0])
                Dtfe.append(Dtf[1])
                periodtf.append(eval(array[i] + ".get_period()"))
            Dpl = eval(str(array[i]) + ".calculate_fourier_diffusivity(n = " + str(j) + ", option = \"pl\")")
            if j%2 == 1:
                Dpls.append(Dpl[0])
                Dple.append(Dpl[1])
                periodpl.append(eval(array[i] + ".get_period()"))
            pass
        plt.scatter(periodtf, Dtfs)
    plt.title("Diffusivities from Fourier Decomposition")
    plt.xlabel("Period/s")
...
def return_fourier_fit_n(array, n = 3, option =  "tf"):
    # for i in range(1, len(array)):
    #     eval(str(array[i]) + ".plot_fourier(" + str(n) + ")")
    #     pass
    plt.figure()
    pool = []
    for i in range(1, n + 1): # each n
        Dtfs = []
        Dtfe = []
        Dpls = []
        Dple = []
        ...
        periodpl = []
        if i%2 == 1:
            for j in range(0, len(array)): # each dataset
                Dtf = eval(str(array[j]) + ".calculate_fourier_diffusivity(n = " + str(i) + ", option = \"tf\")")
                Dpl = eval(str(array[j]) + ".calculate_fourier_diffusivity(n = " + str(i) + ", option = \"pl\")")
                Dtfs.append(Dtf[0])
                Dtfe.append(Dtf[1])
                periodtf.append(eval(array[j] + ".get_period()"))
                Dpls.append(Dpl[0])
                Dple.append(Dpl[1])
                periodpl.append(eval(array[j] + ".get_period()"))
                if option == "tf":
                    pool.append(Dtf[0])
                elif option == "pl":
                    pool.append(Dpl[0])
            ...
            if option == "tf":
                if n < 10:
                    plt.scatter(periodtf, Dtfs, label = "n = %s" % (i))
                    pass
                else:
                    plt.scatter(periodtf, Dtfs)
                    pass
                ...
                if n < 10:
                    plt.scatter(periodpl, Dpls, label = "n = %s" % (i))
                    pass
                else:
                    plt.scatter(periodpl, Dpls)
                    pass
            pass
        pass
    avg = sum(pool)/len(pool)
    err = np.std(pool, ddof=1)/np.sqrt(np.size(pool))
    upper = avg + err
    lower = avg - err
    plt.axhline(avg, linestyle = "dashed", label = "Mean : %s" % float('%.3g' % avg))
    plt.axhline(upper, linestyle = "dotted", alpha = 0.25)
    plt.axhline(lower, linestyle = "dotted", alpha = 0.25)
    plt.axhline(1.24e-7, alpha = 0.75, label = "Actual Value")
    plt.fill_between([0, 1000], [upper, upper], [lower, lower], color = 'blue', alpha = 0.1, label = "Std Err: %s" % float('%.3g' % err))
    plt.title("Diffusivities from Fourier Decomposition(" + option + ")")
    plt.xlabel("Period/s")
    plt.ylabel("Diffusivity/[a.u.]")
    # if option == "pl":
    #     plt.yscale("log")
    plt.grid()
    plt.xlim(0, 1000)
...
fourA.calculate_diffusivities(1, out = True, plotmode = True)
...
#%%
'''Calculating Dpl and Dtf from simple sine fit'''
'''Task 2.5 a)'''
...
'''Task 2.5 b)'''
twoA.calculate_diffusivities(1, out = True, plotmode = False)
# twoB.calculate_diffusivities(1, out = True, plotmode = False)
six.calculate_diffusivities(1, out = True, plotmode = False)
sixteen.calculate_diffusivities(1, out = True, plotmode = False)
...
#%%
'''Calculating Dtf and Dpl from fourier fit'''
'''Task 2.6 a)'''
...
'''Task 2.6 b)'''
def task26b(n = 3):
    print("For dataset 4a:")
    print("Amplitude Phase Form: [amp, phs]")
    for i in range(1, n + 1):
        print("   n = " + str(i) + ": " + str(eval("fourA.get_reduced_amplitude_phase(n = " + str(i) + ")")))
        pass
    print("Diffusivity From Transmission Factor:")
    for i in range(1, n + 1):
        print("   n = " + str(i) + ": " + str(eval("fourA.calculate_fourier_diffusivity(n = " + str(i) + ", option = \"tf\")")))
        pass
    print("Diffusivity From Phase Lag:")
    for i in range(1, n + 1):
        print("   n = " + str(i) + ": " + str(eval("fourA.calculate_fourier_diffusivity(n = " + str(i) + ", option = \"pl\")")))
        pass
    fourA.calculate_diffusivities(n, out = True, plotmode = False)
    fourA.plot_fourier(n)
    return_fourier_fit_n(["fourA"], n, option = "tf")
    return_fourier_fit_n(["fourA"], n, option = "pl")
    print()
    pass
...
def task27a(array, n = 3):
    for run in range(0, len(array)):
        print("For dataset %s:" % (array[run]))
        print("Amplitude Phase Form: [amp, phs]")
        for i in range(1, n + 1):
            print("   n = " + str(i) + ": " + str(eval(str(array[run]) + ".get_reduced_amplitude_phase(n = " + str(i) + ")")))
            pass
        print("Diffusivity From Transmission Factor:")
        for i in range(1, n + 1):
            print("   n = " + str(i) + ": " + str(eval(str(array[run]) + ".calculate_fourier_diffusivity(n = " + str(i) + ", option = \"tf\")")))
            pass
        print("Diffusivity From Phase Lag:")
        for i in range(1, n + 1):
            print("   n = " + str(i) + ": " + str(eval(str(array[run]) + ".calculate_fourier_diffusivity(n = " + str(i) + ", option = \"pl\")")))
            pass
        eval(str(array[run]) + ".calculate_diffusivities(1, out = True, plotmode = False)")
        print()
        eval(str(array[run]) + ".plot_fourier(" + str(n) + ")")
        pass
    pass

# task27a(arraylist)

task27a(arraylist, n = 7)

#%%
'''Task 2.7b)'''
# return_fourier_fit_n(arraylist, n = 3, option =  "tf")
# return_fourier_fit_n(arraylist, n = 3, option =  "pl")
# return_fourier_fit_n(arraylist, n = 5, option =  "tf")
# return_fourier_fit_n(arraylist, n = 5, option =  "pl")
return_fourier_fit_n(arraylist, n = 7, option =  "tf")
return_fourier_fit_n(arraylist, n = 7, option =  "pl")
# return_fourier_fit_n(arraylist, n = 9, option =  "tf")
# return_fourier_fit_n(arraylist, n = 9, option =  "pl")

# return_fourier_fit_n(arraylist, n = 13, option =  "tf")
# return_fourier_fit_n(arraylist, n = 13, option =  "pl")
