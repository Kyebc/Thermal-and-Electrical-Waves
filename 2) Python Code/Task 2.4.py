...
def plot_graph(file_name, period_sig, file_name2 = "False"):
    ...
    if file_name2 != "False":
        ...
        label1 = "Measurement"
    plt.plot(x1, ysquare1, color = "blue", label = "Signal")
    plt.plot(x1, y1, label = "%s" % (label1), color = "red")
    if file_name2 != "False":
        x2, y2 = np.loadtxt("%s" % (file_name2), unpack = True, skiprows = 3)
        plt.plot(x2, y2, label = "%s" % (label2), color = "darkgreen")
...
def cal_amplitude(x = [], y = []):
    ...
    if y == []:
        y = yarray
    global popt, pcov
    popt, pcov = curve_fit(sine, x, y, [2*np.pi/2400, -10, np.pi, 50])
    print(popt)
    print(pcov)
...
#%%
'''Task 2.3 a)'''
plot_graph("thermal_4min_a.txt", 2400)
# plot_graph("thermal_4min_b.txt", 2400)

# plot_graph("thermal_4min_a.txt", 2400, "thermal_4min_b.txt")

# plot_graph("thermal_1min_a.txt", 600, "thermal_1min_b.txt")
# plot_graph("thermal_2min_a.txt", 1200, "thermal_2min_b.txt")
