...
class Thermal():
    ...
    def calc_a_n(self, measurement_mode = 1):
        integral = 0
        for i in range(0, len(self.get_xarray())):
            integral += self.get_yarray()[i] * np.cos(2*np.pi*measurement_mode*self.get_xarray()[i]/self.get_period()) /10
        norm = (2/self.get_period())
        val = norm * integral
        return val
    
    def calc_b_n(self, measurement_mode = 1):
        integral = 0
        for i in range(0, len(self.get_xarray())):
            integral += self.get_yarray()[i] * np.sin(2*np.pi*measurement_mode*self.get_xarray()[i]/self.get_period())
        norm = (2/self.get_period())
        val = norm * integral/10
        return val
    
    def calc_beta_n(self, n = 1):
        a = self.calc_a_n(n)
        b = self.calc_b_n(n)
...
fourA.calculate_diffusivities(1, out = True, plotmode = False)
...
'''Task 2.5 c)'''
arraylist = ["oneA", "twoA", "fourA", "eight"]
